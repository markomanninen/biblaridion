...
from .isopsephy import isopsephy, unicode_isopsephy, to_roman, find_cumulative_indices, preprocess_greek
from .strongs3 import greek
...
manuscripts = {
    "byzantine_2000": {
        "variations": {"var0": "Greek NT: Byzantine/Majority Text (2000)"},
        "source": "greek_byzantine_2000_parsed_utf8"},
    "modern": {
        "variations": {"var0": "Greek OT+NT: Modern Greek Bible"},
        "source": "greek_modern_utf8"},
    "sblgnt": {
        "variations": {"var0": "Greek NT: SBL Edition"},
        "source": "sblgnt"},
    "nestle": {
        "variations": {"var0": "Greek NT: Eberhard Nestle (1904)"},
        "source": "nestle"},
    "tischendorf": {
        "variations": {"var0": "Greek NT: Tischendorf 8th Ed."},
        "source": "greek_tischendorf_utf8"},
    "textus_receptus": {
        "variations": {"var1": "Greek NT: Textus Receptus (Erasmus 1550)",
                       "var2": "Greek NT: Textus Receptus (Stephanus 1894)"},
        "source": "greek_textus_receptus_utf8"},
    "wh_ubs4": {
        "variations": {"var1": "Greek NT: Westcott/Hort (1881)",
                       "var2": "Greek NT: Nestle-Aland 27th/UBS4"},
        "source": "greek_WH_UBS4_parsed_utf8"}
}
...
def parse_verse(verse, manuscript, vocabulary, var = "var0"):
    ...
    verse_words = []
    if manuscript == 'sblgnt' or \
       manuscript == 'modern' or \
       manuscript == 'tischendorf':
        r = regex_word_strong_morph2
        verse = preprocess_greek(verse)
        for tupleitem in [[x] for x in r.findall(verse)]:
            item = []
            # word
            item.append(tupleitem[0])
            # we can find lemma from Strong's dictionary
            #item.append(''.join(find(tupleitem[0], 'word')))
            item.append(''.join([x for x in find(r"(?i)^%s$" % tupleitem[0], 'word').lemma]))
            #item.append(find(tupleitem[0], 'word').lemma)
